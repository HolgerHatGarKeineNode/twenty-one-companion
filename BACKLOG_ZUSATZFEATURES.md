...
- [x] **B.11 Review localization** — audited all views/Livewire for unwrapped user-facing strings. Region/profile/map/onboarding features (the localization expansion) were already clean. Key insight: `de.json` (68) + `en.json` (387) = the full 455-key set — each file deliberately omits its identity-mapping keys (German app strings fall back to the German key; English framework strings fall back to the English key), so de/en are complete via Laravel's key fallback. Only fix: two `label="OTP Code"` (sr-only a11y labels in 2FA challenge + setup modal) now wrapped in `__()`; resolves to "OTP Code" in every language via fallback, so no per-language entries needed.
